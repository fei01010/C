...
StackResult stackInit(Stack* stack, int init_capacity, int max_capacity){
    ...
    if (init_capacity < 1 || max_capacity < 1 || init_capacity > max_capacity) return STACK_MEM_ERR;
...
StackResult stackPush(Stack* stack, char bracket, int position){
    ...
    if (stack->top + 1 >= stack->capacity) {
        ...
        StackElem* p = (StackElem*)realloc(stack->data, sizeof(StackElem) * new_capacity);
        if (p == NULL) return STACK_MEM_ERR;
        stack->data = p;
...
int jsonBracketCheckBasic(const char *json_str) {
    ...
    Stack stack;
    if (stackInit(&stack, 100, 100) != STACK_OK) return 0; 
    ...
    for (int i = 0; json_str[i] != '\0'; ++i) {
        ...
        if (isLeftBracket(ch)) {
            ...
            StackElem topElem;
            if (stackPop(&stack, &topElem) != STACK_OK) {
                stackDestroy(&stack);
                return 0;
            }
            if (!isBracketMatch(topElem.bracket, ch)) {
                stackDestroy(&stack);
                return 0;
            }
        }
...
// 步骤6：销毁栈（stackDestroy）
    if (json_str == NULL || json_str[0] == '\0') return 1;
    Stack stack;
    if (stackInit(&stack, N, N) != STACK_OK) return 0;
    int in_string = 0;
    int escape = 0;
    for (int i = 0; json_str[i] != '\0'; ++i) {
        char ch = json_str[i];
    ...
    int ok = (stackIsEmpty(&stack) && in_string == 0) ? 1 : 0;
    stackDestroy(&stack);
    return ok;
}
